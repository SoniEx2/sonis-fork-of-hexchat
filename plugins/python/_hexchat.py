...
def __get_fields(name):
    return __cstrarray_to_list(lib.hexchat_list_fields(lib.ph, name))


@lru_cache(64)
def __decoded_str(string):
    return __decode(string)


def get_lists():
    return [__decoded_str(field) for field in __get_fields(b'lists')]
    ...
    while lib.hexchat_list_next(lib.ph, list_) is 1:
        ...
        for field in fields:
            ...
            if getter is not None:
                field_name = field[1:]
                setattr(item, __decoded_str(field_name), getter(field_name))
        ret.append(item)
...
class Context:
    ...
    def __change_context(self):
        ...
        if not self.set():
            ...
            print('Context object refers to closed context, ignoring call')
            return
        yield
